...
const IconDivide = () => (
  <svg
    ...
    <path
      fillRule="evenodd"
      d="M3 10a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1zM10 4a1 1 0 100 2 1 1 0 000-2zm0 10a1 1 0 100 2 1 1 0 000-2z"
      clipRule="evenodd"
    />
  </svg>
);
...
const ItemConsumido = ({ restauranteId, item, onEliminar }) => {
              ...
              items: r.items.map((i) => {
                if (i.id === item.id) {
                  return {
                    ...
                    personasAsignadas:
                      cantidad !== i.cantidad ? [] : i.personasAsignadas,
                  };
                }
                return i;
              }),
  ...
  const dividirEntreSeleccionados = () => {
    ...
    const nuevasAsignaciones = item.personasAsignadas.map((asignacion) => ({
      ...asignacion,
      cantidad: personasSeleccionadas.includes(asignacion.personaId)
        ? cantidadPorPersona
        : 0,
    }));
      ...
      prev.map((r) => {
        if (r.id === restauranteId) {
          return {
            ...
            items: r.items.map((i) => {
              if (i.id === item.id) {
                const asignacionExistente = i.personasAsignadas.find(
                ...
                if (asignacionExistente) {
                  // Eliminar asignación
                  nuevasAsignaciones = i.personasAsignadas.filter(
                    (a) => a.personaId !== personaId
                  );
                } else {
                  // Agregar asignación
            ...
            items: r.items.map((i) => {
              if (i.id === item.id) {
                if (cantidad <= 0) {
                  ...
                  return {
                    ...i,
                    personasAsignadas: i.personasAsignadas.filter(
                      (a) => a.personaId !== personaId
                    ),
                  };
                }
          ...
          return {
            ...
            items: r.items.map((i) => {
              if (i.id === item.id) {
                ...
                const personasSeleccionadas = i.personasAsignadas.map(
                  (a) => a.personaId
                );
                ...
                const unidadesPorPersona = Math.ceil(
                  unidadesPorAsignar / personasDisponibles.length
                );
                ...
                const nuevasAsignaciones = [
                  ...
                  ...personasDisponibles.map((persona, idx) => {
                    ...
                    const cantidadAsignada = esUltima
                      ? Math.min(
                          unidadesPorAsignar - unidadesPorPersona * idx,
                          unidadesPorPersona
                        )
  ...
  return (
    <div
      className={`border p-3 rounded ${
        tipoAsignacion === "compartido" ? "bg-blue-50" : "bg-gray-50"
      }`}>
      ...
      <div className="mb-2">
        <div className="flex justify-between items-center">
          ...
          <div className="text-sm text-gray-600">
            ...
            <span className="text-xs">(sin propina)</span>
          </div>
        </div>
        ...
        <div
          className={`p-2 mt-1 mb-2 text-xs rounded ${
        ...
        <div className="flex items-center gap-2">
          ...
          {item.personasAsignadas.length > 0 && (
            <span className="text-xs bg-blue-100 px-2 py-1 rounded flex items-center">
              ...
              {item.personasAsignadas.length}{" "}
              {item.personasAsignadas.length === 1 ? "persona" : "personas"}
            </span>
          )}
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-2 mb-3">
        <button
          ...
          <svg
            className={`w-3 h-3 ml-1 transition-transform ${
              mostrarDetalles ? "transform rotate-180" : ""
            }`}
            fill="currentColor"
            viewBox="0 0 20 20">
            <path
        ...
        <button
          ...
          <svg
            className={`w-3 h-3 ml-1 transition-transform ${
              mostrarAsignaciones ? "transform rotate-180" : ""
            }`}
            fill="currentColor"
            viewBox="0 0 20 20">
            <path
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {personas.map((persona) => {
              ...
              return (
                <div
                  key={persona.id}
                  className={`flex items-center space-x-2 border p-1.5 rounded text-sm ${
                  ...
                  {cantidadAsignada > 0 && (
                    <div className="flex items-center space-x-1">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          actualizarCantidadAsignada(persona.id, e.target.value)
                        }
